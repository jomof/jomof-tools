...
<a id="buffer.set_default_fold_display_text"></a>
### `buffer.set_default_fold_display_text`(*buffer, text*)
...
### `textadept.editing.show_documentation`(*pos, case\_insensitive*)
...
* *`case_insensitive`*: Optional flag that indicates whether or not to search
...
### `ui.command_entry.run`(*f, mode\_keys, lexer, height*)
...
* *`f`*: Optional function to call upon pressing `Enter` in the command
  ...
  argument.
* *`mode_keys`*: Optional table of key bindings to respond to. This is in
...
<a id="ui.find.find_incremental_keys"></a>
